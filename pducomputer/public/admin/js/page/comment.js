...
$(function() {
    datatables_columns = [{
        ...
        width: 70,
        template: function (t) {
            return '<strong class="m--font-success">'+t.count_like+'<i class="fa fa-thumbs-o-up"></i></strong>'
        }
    },{
        field: "report",
        title: "Report",
        width: 70,
        template: function (t) {
            return '<strong class="m--font-danger">'+t.report+'<i class="fa fa-warning"></i></strong>'
        }
    },{
        field: "count_star",
        title: "Star",
        width: 70,
        template: function (t) {
            return '<strong class="m--font-warning">'+t.count_star+'<i class="fa fa-star"></i></strong>'
        }
    }, {
        field: "is_status",
        title: "Status",
    ...
    AJAX_CRUD_MODAL.tinymce();//khung editor
    SEO.init_slug();
        ...
        AJAX_CRUD_MODAL.edit(function () {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if(response.data_info.file_attach_thumb != undefined){
                        let file_attach = modal_form.find('.file_attach');
                        file_attach.find("img").attr('src',base_url+"public/"+response.data_info.file_attach_thumb);
                     }
