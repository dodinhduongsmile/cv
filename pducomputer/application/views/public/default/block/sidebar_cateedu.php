<?php 
    ...
    $home_left = getCateChild($allcate,0);
...
?>
    ...
    <div class="sidebar">
      ...
      <ul class="menu-aside">
        <?php if (!empty($allcate)) foreach ($allcate as $key => $value){
          if($value->parent_id == 0){
            //hiển thị cấp 0
             unset($allcate[$key]);
             $child_home_left = getCateChild($allcate,$value->id);
            
        ?>
        <li class="<?php echo !empty($child_home_left)? 'has-dropdown' : ''; ?>">
            <h3><a href="<?php echo base_url($urlcate[$value->type].$value->id."_".$value->slug.'.html'); ?>" title="<?php echo $value->title; ?>"><?php echo $value->title; ?></a></h3>

            <?php if (!empty($child_home_left)){ ?>
              <ul class="<?php echo $classsidebar; ?> nav-aside">
                  <?php foreach ($child_home_left as $key1 => $value1){
                    if($value1->parent_id == $value->id){
                      //hiển thị cấp 1
                       unset($allcate[$key1]);
                       $child_home_left2 = getCateChild($allcate,$value1->id);

                  ?> 

                    <li class="<?php echo !empty($child_home_left2)? 'has-dropdown' : ''; ?>">
                      <h3><a href="<?php echo base_url($urlcate[$value1->type].$value1->id."_".$value1->slug.'.html'); ?>" title="<?php echo $value1->title; ?>"><?php echo $value1->title; ?></a></h3>
                        <?php if (!empty($child_home_left2)){ ?>
                            <ul class="<?php echo $classsidebar; ?> nav-aside">
                              <?php foreach ($child_home_left2 as $key2 => $value2){ 
                                unset($allcate[$key2]);
                              ?>
                              <h3><a href="<?php echo base_url($urlcate[$value2->type].$value2->id."_".$value2->slug.'.html'); ?>" title="<?php echo $value2->title; ?>"><?php echo $value2->title; ?></a></h3>
                              <?php }; ?>
                            </ul>
...
<div class="new_highlight">
   ...
   <ul>
    <?php if(!empty($datapost)) foreach($datapost as $item): ?>
      <li class="clearfix">
         <a href="<?php echo get_url_post($item); ?>"><img src="<?php echo getThumbnailnoajax($item->thumbnail,230,230); ?>" alt="<?php echo $item->title; ?>"></a>
