...
class Edu extends Admin_Controller
{
    protected $_data;
    protected $_user;
    ...
    public function __construct()
    {
        parent::__construct();
        //tải thư viện
        $this->load->model(['edu_model','users_model']);
        $this->_data         = new Edu_model();
        $this->_user         = new Users_model();
    }

    public function index(){
        $data['heading_title'] = "Quản lý khóa học";
    ...
    public function ajax_list(){
        ...
        if(!empty($listData)) foreach ($listData as $item) {
            ...
            $row['checkID']    = $item->id;
            $row['id']         = $item->id;
            $row['title']        = '<a href="'.get_url_edu($item).'" target="_blank">'.$item->title.'</a>';
    ...
    private function save_category($id, $data) {
        $this->_data->delete(['edu_id'=>$id],'edu_category');
        if(!empty($data)) foreach ($data as $category_id){
            $tmp = ["edu_id" => $id, 'category_id' => $category_id];
            $data_category[] = $tmp;
        }

        if(!$this->_data->insertMultiple($data_category, 'edu_category')){
            $message['type'] = 'error';
    ...
    public function ajax_add(){
        ...
        $data = $this->_convertData();
         ...
         if(!empty($data['listyoutube'])){
            //youtube
            $listyoutube = $data['listyoutube'];
            $listyoutube_name = $data['listyoutube']['name'];
            $data_youtube = [];
            foreach ($listyoutube_name as $key => $value){
                
                $data_youtube[] = [
                    "name" => $listyoutube['name'][$key],
                    "link" => $listyoutube['link'][$key],
                    "id" => $listyoutube['id'][$key],
                    "time" => $listyoutube['time'][$key]
                ];
            }
            $data['listyoutube'] = json_encode($data_youtube);
         }else{
            $data['listyoutube'] = "";
         }
        ...
        if($id = $this->_data->save($data)){
            $note   = 'Thêm edu có id là : '.$id;
            //$this->addLogaction('edu',$data,$id,$note,'Add');
    ...
    public function ajax_edit(){
        ...
        if(!empty($id)){
            ...
            // $output['data_info']->listdrive = json_decode($data_info->listdrive);
    ...
    public function ajax_update(){
        ...
        if($this->_data->update(['id' => $id],$data, $this->_data->table)){
            $this->save_category($id, $data_category);
            $note   = 'Update edu có id là : '.$id;
            //$this->addLogaction('edu',$data_old,$id,$note,'Update');
            $message['type'] = 'success';
    ...
    public function ajax_delete(){
        ...
        if($response != false){
            ...
            $this->_data->deleteArray('edu_id',$ids,'edu_category');
            $message['type'] = 'success';
